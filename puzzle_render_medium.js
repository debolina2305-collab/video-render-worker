'use strict';
// ═══════════════════════════════════════════════════════════════════════
// PUZZLE RENDER MEDIUM — short-no-intro flow + an answer-reveal beat
//
// This is puzzlerenderswithoutintro.js with ONE change: an ANSWER REVEAL
// step (borrowed from puzzle_render_long.js's Step 10 — .answer-slide,
// correct option highlighted green / wrong ones dimmed, explanation panel,
// confetti) is spliced in between "time's up" and the CTA4 screen.
//
//   1. Polls the `puzzle` table (via puzzleAssigner.js), format="medium"
//      (medium_status: pending_medium → rendering_medium → done_medium,
//      video URL written to medium_video_url).
//   2. Reads puzzle_template.html, resolves puzzleRenderers.js for the SVG.
//   3. Artifacts named *_puzzle_medium_video.mp4.
//
// FLOW (all durations 100% dynamic from actual audio file lengths):
//
//  Step 3   — Q+OPTIONS screen (no host hook clip — starts here directly).
//             Avatar strip: human circle silent/idle video; dog circle
//             talks while TTS plays (question + "N sec, time starts now!").
//
//  Step 4   — COUNTDOWN screen. Avatar strip: both human and dog silent.
//             5-second countdown, 50/50 elimination at 2.5s.
//
//  Step 5   — TIMEUP screen. Host circle speaks the "time's up" line
//             (first half of the avatar_assets cta4 clip, split at
//             timeup_split_sec). Dog: reaction clip / idle.
//
//  Step 5.5 — ANSWER REVEAL (NEW). Screen switches to .answer-slide:
//             correct option highlighted, wrong ones dimmed, explanation
//             panel, confetti. Dog "talks" while TTS speaks the correct
//             answer; host circle stays on its silent idle loop.
//
//  Step 6   — CTA screen ("write your answer in comments"). Host circle
//             speaks the CTA4 line (second half of the same avatar clip).
//
// ASSETS (from avatar_assets Supabase table, R2 URLs):
//   silent  — looping idle .mp4 for human circle (steps 3/4/answer-reveal)
//   cta4    — ONE clip containing BOTH the time's-up AND cta4 lines, split
//             at timeup_split_sec; steps 5 and 6 each use one half.
//   Dog     — MP4 reaction clips per section, or pure CSS fallback.
// ═══════════════════════════════════════════════════════════════════════
...
const CACHE_DIR         = '/tmp/puzzle_cache_short';
...
const GAP_ANSWER        = 0.35; // silence before the answer-reveal sfx/TTS starts
...
// TTS_VOLUME_GAIN: "increase TTS volume by 200%" == make it 3x as loud
// (100% original + 200% more = 300% = 3.0x gain). Applied as a post-process
// ffmpeg volume boost (with a limiter so it doesn't hard-clip) — this works
// regardless of what the edge-tts voice's own default loudness is.
const TTS_VOLUME_GAIN = 3.0;

async function boostTtsVolume(filePath) {
  const tmp = `${filePath}.boost.mp3`;
  try {
    await ffmpeg(
      `-y -i "${filePath}" -af "volume=${TTS_VOLUME_GAIN},alimiter=limit=0.95" ` +
      `-ar 44100 -ac 1 -acodec libmp3lame "${tmp}"`,
      'ttsVol'
    );
    await fs.rename(tmp, filePath);
  } catch (e) {
    console.warn(`[TTS-VOL] boost failed (non-fatal, using unboosted audio): ${e.message.slice(0,100)}`);
  }
}
...
// ═══════════════════════════════════════════════════════════════════════
async function buildMediumVideo(quiz, workDir) {
  // FIXED 8-second countdown for every puzzle rendered by this format —
  // no longer varies by puzzle_type.
  ...
  // Answer-reveal colour classes: correct option → green badge/border,
  // everything else → dimmed. Only ever painted on .answer-slide (the
  // other screens use opt0_class../opt-eliminated for the 50/50, not
  // rev*_class), so this is safe to compute unconditionally.
  ...
  const [countdownFile, bgFile, sfxFile, correctSfxFile] = await Promise.all([
    ...
    download(quiz.correct_answer_sfx_audio_url, `sh_correctsfx_${quiz.id}`),
  ]);
...
:root { --topic-photo-url: url("file://${dl}"); }
  ...
  let html = await fs.readFile(path.join(__dirname, 'puzzle_template.html'), 'utf8');
  const R = {
    '{{theme_css}}':         themeCss,
  ...
  // ── PUZZLE VISUAL: render the SVG from puzzle_spec and inject into template ──
  // puzzleRenderers.js is a CommonJS module in the same directory.
  // We require() it here (Node 20 can require CJS from CJS safely).
  let puzzleSvgHtml = '';
  try {
    const { renderPuzzle } = require('./puzzleRenderers');
    const spec    = typeof quiz.puzzle_spec === 'string'
      ? JSON.parse(quiz.puzzle_spec)
      : (quiz.puzzle_spec || {});
    const pzType  = quiz.puzzle_type || 'number_sequence';
    const pzOpts  = {
    ...
    if (result.ok) {
      puzzleSvgHtml = result.svg;
      console.log(`[PZ-SHORT] SVG rendered: type=${pzType} bytes=${puzzleSvgHtml.length}`);
      if (result.warnings?.length) console.warn('[PZ-SHORT] SVG warnings:', result.warnings);
    } else {
      console.warn('[PZ-SHORT] SVG not ok:', result.warnings);
      puzzleSvgHtml = result.svg; // fallback SVG still shows an error card
    }
    // Write the SVG back to the puzzle row so blog/publisher can reuse it
    if (puzzleSvgHtml) {
      fetchSupabase(`puzzle?id=eq.${quiz.id}`, {
        method: 'PATCH',
        body: JSON.stringify({ puzzle_svg: puzzleSvgHtml, updated_at: new Date().toISOString() })
      }).catch(e => console.warn('[PZ-SHORT] SVG back-write failed:', e.message));
    }
  } catch (e) {
    console.warn('[PZ-SHORT] puzzleRenderers require/render failed:', e.message);
  }
...
.short-fmt .answer-slide {
  padding-top:    ${TOP_STRIP_H + 24}px !important;
  padding-bottom: 60px !important;
  box-sizing: border-box !important;
}

/* ── ANSWER REVEAL (medium-only beat) — shrink to fit under the avatar
   top-strip, which eats ~320px that the long-format answer slide doesn't
   have to budget for. ── */
.short-fmt .answer-badge {
  font-size:   44px !important;
  margin-bottom: 22px !important;
}
.short-fmt .answer-explanation-heading {
  font-size: 24px !important;
  padding:   12px 20px !important;
}
.short-fmt .answer-explanation-body {
  font-size:   28px !important;
  line-height: 1.35 !important;
  padding:     16px 20px !important;
}
.short-fmt .answer-slide .qp-option {
...
/* ── NO-INTRO VARIANT: remove the generic "{{question}}" text card from the
   top of every screen (e.g. "What number replaces the question mark?"). It
   just duplicated the puzzle visual's own on-image title and ate vertical
   space. The puzzle SVG itself carries all the framing the viewer needs. ── */
.short-fmt .qp-card-question { display: none !important; }
...
/* ── OPTIONS container: 2x2 GRID instead of a tall single column ──────────
   Frees up vertical space (especially now that the top question card is
   hidden) and reads faster on a vertical Shorts frame. ── */
...
/* ── HINT: shown 1s AFTER the countdown starts — pinned to the BOTTOM of the
   viewable/safe area (not immediately under the options). .content is a
   column flex box, so margin-top:auto on its last child pushes the hint all
   the way down to just above the reserved YouTube-UI safe zone, no matter
   how much (or little) space the puzzle visual + 2x2 options take up above
   it. ── */
.short-fmt .qp-hint {
  ...
  margin: auto 34px 0 !important;   /* margin-top:auto → pinned to bottom */
...
/* ── Answer colour reveal is ONLY painted on .answer-slide (the other
   screens use opt-eliminated/opt-elim-target for the 50/50, never
   .correct/.wrong) — so no need to neutralise it here like the short
   no-intro variant does. The base theme's .qp-option.correct/.wrong
   rules apply as-is on the answer-slide. ── */
...
/* "Follow and Subscribe" is longer than "Subscribe" — shrink + allow wrap
   so it still fits inside the pill instead of overflowing. */
.short-fmt .comment-cta-screen .cta-pill-sub {
...
${AVATAR_CSS}`;
  ...
  // ══ STEP 1+2 — REMOVED IN THIS NO-INTRO VARIANT ════════════════════
  // The full-screen host "hook" clip (and its hook-slide fallback) is
  // intentionally skipped so the video starts DIRECTLY on the Q+Options
  // puzzle screen (Step 3 below). The rounded host + mascot circles in the
  // top corner are untouched — they still appear from Step 3 onward exactly
  // as before, since that compositing logic lives in Step 3/4/5+6.
  console.log('[SHORT-NOINTRO] Step 1+2 skipped — starting directly on the puzzle screen.');
  ...
  // ══ STEP 5 — TIMEUP ═════════════════════════════════════════════════
  // The avatar_assets cta4 clip is ONE recording containing BOTH the
  // time's-up line and the CTA4 line, split at timeup_split_sec. We cut
  // it into its two halves here — the first half is this step's host
  // video + audio, the second half feeds Step 6 below. In between we
  // splice in the NEW answer-reveal beat (Step 5.5).
  console.log('[MEDIUM] -- Step 5: timeup');

  await setAvatarMode(page, 'human_speaking');
  await showScreen(page, '.pre-reveal-slide');

  let step5Audio, step5Dur;
  let hostCta4TimeupSeg = null, hostCta4CtaSeg = null;
  let splitAt = 0, cta4TotalDur = 0;

  if (hostCta4File && await fileExists(hostCta4File)) {
    cta4TotalDur = Math.max(await videoDur(hostCta4File), 1.5);

    // When the clip switches from the time's-up line to the CTA4 line.
    // Prefer the DB value; else 45% of the way through.
    splitAt = (avatarSet.timeupSplitSec != null && avatarSet.timeupSplitSec > 0
               && avatarSet.timeupSplitSec < cta4TotalDur)
      ? Number(avatarSet.timeupSplitSec)
      : cta4TotalDur * 0.45;
    console.log(`[MEDIUM] cta4 clip ${cta4TotalDur.toFixed(2)}s, split at ${splitAt.toFixed(2)}s`);

    // Video-only halves (no audio needed — compositeHumanCircle never
    // uses the human clip's own audio track, only the UI recording's).
    hostCta4TimeupSeg = path.join(workDir, 'sh_cta4_seg_timeup.mp4');
    await ffmpeg(
      `-y -i "${hostCta4File}" -t ${splitAt} -an ` +
      `-c:v libx264 -crf 27 -preset faster -pix_fmt yuv420p -r 30 "${hostCta4TimeupSeg}"`,
      'cta4_seg_timeup'
    );
    hostCta4CtaSeg = path.join(workDir, 'sh_cta4_seg_cta.mp4');
    await ffmpeg(
      `-y -i "${hostCta4File}" -ss ${splitAt} -an ` +
      `-c:v libx264 -crf 27 -preset faster -pix_fmt yuv420p -r 30 "${hostCta4CtaSeg}"`,
      'cta4_seg_cta'
    );

    // Audio: the same clip's own soundtrack, trimmed at the same split.
    step5Audio = path.join(workDir, 'sh_step5_audio.mp3');
    await ffmpeg(
      `-y -i "${hostCta4File}" -t ${splitAt} -vn -ar 44100 -ac 1 -b:a 128k -acodec libmp3lame "${step5Audio}"`,
      'step5_audio'
    );
    step5Dur = Math.max(await audioDur(step5Audio), splitAt, 0.8);
  } else {
    // No cta4 clip — TTS fallback for both lines (naturally already split
    // into two separate files, one per step).
    console.log('[MEDIUM] Step 5: no cta4 host clip — TTS fallback');
    step5Audio = path.join(workDir, 'sh_timeup_tts.mp3');
    await tts(quiz.timeup_text || "Time's up!", voice, step5Audio, 3);
    step5Dur = Math.max(await audioDur(step5Audio), 1.2);
  }

  const step5Ui = await recordUiWithEvents(page, step5Audio, step5Dur, workDir, 'sh_step5_ui', []);

  let step5Final = step5Ui;
  const step5Host = hostCta4TimeupSeg
    || (hostSilentFile && await fileExists(hostSilentFile) ? hostSilentFile : null);
  if (step5Host) {
    const cp = path.join(workDir, 'sh_step5_host.mp4');
    await compositeHumanCircle(step5Ui.path, step5Host, step5Dur, cp, hostSlot);
    step5Final = { path: cp, dur: step5Dur };
    console.log(`[MEDIUM] Step 5: ${hostCta4TimeupSeg ? 'cta4-timeup-half' : 'silent'} host composited`);
  }
  if (dogCta4File_d && await fileExists(dogCta4File_d)) {
    const cp = path.join(workDir, 'sh_step5.mp4');
    // loop=false: reaction clip plays once, matching the original combined-step behaviour
    await compositeDogCircle(step5Final.path, dogCta4File_d, step5Dur, cp, dogSlot, false);
    step5Final = { path: cp, dur: step5Dur };
    console.log('[MEDIUM] Step 5: dog cta4 clip composited');
  }
  pushClip(step5Final, true);

  // ══ STEP 5.5 — ANSWER REVEAL (NEW) ══════════════════════════════════
  // Mirrors puzzle_render_long.js's Step 10: the .answer-slide already
  // baked into puzzle_template.html (and used by the long renderer) shows
  // the correct option highlighted green, the others dimmed, an
  // explanation panel and confetti — all wired up above via the
  // {{correct_answer}} / {{explanation_1}} / {{rev*_class}} placeholders.
  // TTS speaks the correct answer while the mascot "talks"; the host
  // circle stays on its silent idle loop (no dedicated avatar clip for
  // this beat exists in avatar_assets).
  console.log('[MEDIUM] -- Step 5.5: answer reveal');

  await setAvatarMode(page, 'dog_speaking');
  await showScreen(page, '.answer-slide');

  const ansParts = [];
  const ansGap = path.join(workDir, 'sh_ans_gap.mp3');
  await silence(GAP_ANSWER, ansGap);
  ansParts.push(ansGap);
  if (correctSfxFile) {
    ansParts.push(correctSfxFile);
    const sg3 = path.join(workDir, 'sh_ans_sfxgap.mp3');
    await silence(0.15, sg3);
    ansParts.push(sg3);
  }
  const correctTts = path.join(workDir, 'sh_correct_tts.mp3');
  await tts(correct || 'The correct answer', voice, correctTts, 2);
  ansParts.push(correctTts);
  const answerAudio = path.join(workDir, 'sh_answer_audio.mp3');
  await concatAudio(ansParts, answerAudio, workDir);
  const answerDur = Math.max(await audioDur(answerAudio), 2.0);

  const answerUi = await recordUiWithEvents(page, answerAudio, answerDur, workDir, 'sh_answer_ui', []);

  let answerFinal = answerUi;
  if (hostSilentFile && await fileExists(hostSilentFile)) {
    const cp = path.join(workDir, 'sh_answer_host.mp4');
    await compositeHumanCircle(answerUi.path, hostSilentFile, answerDur, cp, hostSlot);
    answerFinal = { path: cp, dur: answerDur };
    console.log('[MEDIUM] Step 5.5: silent host composited');
  }
  if (dogSpeakingFile && await fileExists(dogSpeakingFile)) {
    const cp = path.join(workDir, 'sh_answer.mp4');
    // loop=true: speaking clip loops to fill the exact TTS duration
    await compositeDogCircle(answerFinal.path, dogSpeakingFile, answerDur, cp, dogSlot, true);
    answerFinal = { path: cp, dur: answerDur };
    console.log(`[MEDIUM] Step 5.5: dog speaking clip looped to ${answerDur.toFixed(2)}s`);
  }
  pushClip(answerFinal, true);

  // ══ STEP 6 — CTA4 ═══════════════════════════════════════════════════
  console.log('[MEDIUM] -- Step 6: CTA4');

  await setAvatarMode(page, 'human_speaking');

  let step6Audio, step6Dur;
  if (hostCta4CtaSeg && await fileExists(hostCta4CtaSeg)) {
    step6Audio = path.join(workDir, 'sh_step6_audio.mp3');
    await ffmpeg(
      `-y -i "${hostCta4File}" -ss ${splitAt} -vn -ar 44100 -ac 1 -b:a 128k -acodec libmp3lame "${step6Audio}"`,
      'step6_audio'
    );
    step6Dur = Math.max(await audioDur(step6Audio), Math.max(0.6, cta4TotalDur - splitAt), 1.0);
  } else {
    console.log('[MEDIUM] Step 6: no cta4 host clip — TTS fallback');
    step6Audio = path.join(workDir, 'sh_cta4_tts.mp3');
    await tts(quiz.cta4_text || 'Write your answer in the comments!', voice, step6Audio, 3);
    step6Dur = Math.max(await audioDur(step6Audio), 1.5);
  }
  ...
  // animation-delay up to 2.8s in the template. CTA4 is now its own full
  // step (not a mid-clip switch), so we re-time once, up front, to fit
  // whatever this step's actual duration turns out to be.
  const ctaInfo = await page.evaluate((remaining) => {
    const screen = document.querySelector('.screen.active');
    if (!screen) return { scaled: 0, scale: 1 };
    const LAST_DELAY = 2.8;                    // longest original delay (the arrow)
    const target = remaining * 0.65;           // leave ~35% breathing room after landing
    ...
    return { scaled: els.length, scale: +scale.toFixed(3) };
  }, step6Dur);
  console.log(`[MEDIUM] Step 6: CTA slide; ${ctaInfo.scaled} element(s) re-timed (scale=${ctaInfo.scale})`);

  const step6Ui = await recordUiWithEvents(page, step6Audio, step6Dur, workDir, 'sh_step6_ui', []);

  let step6Final = step6Ui;
  const step6Host = hostCta4CtaSeg
    || (hostSilentFile && await fileExists(hostSilentFile) ? hostSilentFile : null);
  if (step6Host) {
    const cp = path.join(workDir, 'sh_step6_host.mp4');
    await compositeHumanCircle(step6Ui.path, step6Host, step6Dur, cp, hostSlot);
    step6Final = { path: cp, dur: step6Dur };
    console.log(`[MEDIUM] Step 6: ${hostCta4CtaSeg ? 'cta4-cta-half' : 'silent'} host composited`);
  }
  if (dogCta4File_d && await fileExists(dogCta4File_d)) {
    const cp = path.join(workDir, 'sh_step6.mp4');
    await compositeDogCircle(step6Final.path, dogCta4File_d, step6Dur, cp, dogSlot, false);
    step6Final = { path: cp, dur: step6Dur };
    console.log('[MEDIUM] Step 6: dog cta4 clip composited');
  }
  ...
  console.log(`[MEDIUM] Measured ${measured.toFixed(2)}s → using ${finalDur.toFixed(2)}s`);

  let finalPath = concatenated;
  try {
    finalPath = await applyBgMusic(concatenated, finalDur, voiceRanges, resolvedBgFile, workDir);
  } catch (e) {
    console.warn(`[MEDIUM][BG] Failed (non-fatal): ${e.message}`);
  }
...
async function processJobs() {
  console.log('[PZ-MEDIUM] Starting — puzzle medium format (short no-intro + answer reveal)');

  const result = await pollPuzzleFormat(fetchSupabase, patchForAssigner, 'medium', '[PZ-MEDIUM]');
  if (!result) return;
  const { puzzle: quiz, cfg } = result;   // alias puzzle → quiz so all helpers below work unchanged

  const workDir = `/tmp/pz_medium_${uuidv4()}`;
  await ensureDir(workDir);

  try {
    const { videoPath, durationSec } = await withTimeout(
      buildMediumVideo(quiz, workDir), TIMEOUT_JOB, `buildMediumVideo ${quiz.id}`
    );
    const stats  = await fs.stat(videoPath);
    const sizeMb = parseFloat((stats.size / (1024*1024)).toFixed(2));
    console.log(`[MEDIUM-WORKER] ✓ ${durationSec.toFixed(1)}s ${sizeMb}MB`);

    const artifactPath = `/tmp/${quiz.id}_puzzle_medium_video.mp4`;
    await fs.copyFile(videoPath, artifactPath);
    await fs.writeFile('/tmp/medium_artifact_ready', artifactPath);

    let mediumVideoUrl = null;
    if (R2_CONFIGURED) {
      try {
        const buf = await fs.readFile(artifactPath);
        const key = `puzzles/${quiz.id}_medium.mp4`;
        await withTimeout(
          s3Client.send(new PutObjectCommand({ Bucket:R2_BUCKET, Key:key, Body:buf, ContentType:'video/mp4' })),
          60000, 'R2 medium upload'
        );
        mediumVideoUrl = `${R2_PUBLIC_URL.replace(/\/$/, '')}/${key}`;
        console.log(`[R2] ${mediumVideoUrl}`);
      } catch (e) { console.warn(`[R2] Upload failed: ${e.message}`); }
    }

    // markDone handles the status column (medium_status) + video URL
    // column (medium_video_url) atomically
    await markPuzzleDone(patchForAssigner, quiz.id, cfg, mediumVideoUrl);

    await fs.rm(workDir, { recursive: true, force: true }).catch(() => {});
    console.log(`[PZ-MEDIUM] Artifact: ${artifactPath}`);

  } catch (err) {
    console.error('[PZ-MEDIUM] FAILED:', err.message);
    console.error(err.stack?.slice(0, 600) || '');
    await markPuzzleError(patchForAssigner, quiz.id, cfg, err.message);
    await fs.rm(workDir, { recursive: true, force: true }).catch(() => {});
    throw err;
  }
}
...
processJobs()
  .then(() => { console.log('[PZ-MEDIUM] Done.'); process.exit(0); })
  .catch(err => { console.error('[PZ-MEDIUM] Fatal:', err.message); process.exit(1); });
