...
async function buildShortVideo(quiz, workDir) {
  ...
  if (quiz.puzzle_background_image_url && String(quiz.puzzle_background_image_url).startsWith('http')) {
    const dl = await download(quiz.puzzle_background_image_url, `sh_bgphoto_${quiz.id}`).catch(e => {
      console.warn(`[SHORT-BG] download() error: ${e.message.slice(0, 60)}`);
      return null;
    });
...
:root { --topic-photo-url: url("file://${dl}"); }
      ...
      videoPhotoClass = '';
      console.log(`[SHORT-BG] Photo overlay active: ${quiz.puzzle_background_image_url.slice(0, 70)}`);
    } else {
      console.log('[SHORT-BG] Download failed — overlay hidden (.no-photo).');
    }
  } else {
    console.log('[SHORT-BG] No puzzle_background_image_url on this row — overlay hidden.');
  }
  // ── Theme ──────────────────────────────────────────────────────────
...
async function processJobs() {
  console.log('[PZ-SHORT] Starting — puzzle short format');

  const result = await pollPuzzleFormat(fetchSupabase, patchForAssigner, 'short', '[PZ-SHORT]');
  if (!result) return;
  ...
  try {
    ...
    const artifactPath = `/tmp/${quiz.id}_puzzle_short_video.mp4`;
    ...
    if (R2_CONFIGURED) {
      try {
        ...
        const key = `puzzles/${quiz.id}_short.mp4`;
        await withTimeout(
