...
async function loadLLMConfig() {
  ...
  console.log(`[PZ-BLOG] LLM config: model=${model} endpoint=${endpoint.split('/')[2]}`);
...
async function pingIndexNow(urls) {
  ...
  try {
    ...
    if (res.status === 200) {
      console.log(`[PZ-BLOG] ✓ IndexNow pinged ${urls.length} URL(s) → Bing will index within 24h`);
    } else {
      console.warn(`[PZ-BLOG] IndexNow returned HTTP ${res.status} — URLs may not be submitted`);
    }
  } catch (e) {
    console.warn(`[PZ-BLOG] IndexNow ping failed (non-fatal): ${e.message}`);
  }
}
...
function buildPrompt(job, quizRows) {
  ...
  console.log(`[PZ-BLOG] Layout mode: ${layoutMode} (${layout.name}) for topic="${topic}"`);
...
// ─────────────────────────────────────────────
async function run() {
  console.log('[PZ-BLOG] Blog generator starting...');
  ...
  console.log(`[PZ-BLOG] ${existingBlogQuizIds.length} quiz_ids already have blogs`);

  let quizFilter = 'puzzle?puzzle_enriched=eq.true&is_active=eq.true' +
    '&select=id,topic,topic_slug,niche,quiz_no,niche_challenge_no,' +
    'question_1,options_1,correct_answer_1,explanation_1,blog_slug,' +
    'puzzle_type,puzzle_svg,' +
  ...
  if (!pendingQuizzes?.length) {
    console.log('[PZ-BLOG] No enriched puzzles found.');
    return;
  }
  console.log(`[PZ-BLOG] Found ${pendingQuizzes.length} quiz rows without blogs`);
  ...
  if (!topicMap.size) {
    console.log('[PZ-BLOG] All quizzes already have blogs.');
    return;
  }
  ...
  console.log(`[PZ-BLOG] ${topicMap.size} topics need blogs — processing ${topicsToProcess.length} this run`);
  ...
  for (const [topicKey, { quiz: primaryQuiz, rows: extraRows }] of topicsToProcess) {
    ...
    console.log(`\n[PZ-BLOG] Generating blog for: "${primaryQuiz.topic}" (${allRows.length} quiz rows)`);

    try {
      const queueRows = await supabase(
        `puzzle_queue?trnding_topic=ilike.${encodeURIComponent(primaryQuiz.topic)}` +
        `&select=id,trnding_topic,niche,searched_text,payload,topic_image_url&limit=1`
      );
      const job = queueRows?.[0];

      if (!job) {
        console.warn(`[PZ-BLOG] No quiz_queue row found for topic="${primaryQuiz.topic}" — using quiz data only`);
      } else {
        ...
        if (words < 400) console.warn(`[PZ-BLOG] ⚠️ Thin Tavily data: only ${words} words — blog quality may be lower.`);
        else console.log(`[PZ-BLOG] Tavily data: ${words} words, ${imgs} images, ${srcs} sources ✓`);
      }
      ...
      console.log('[PZ-BLOG] Calling DeepSeek via quiz_generation_settings config...');
      const rawResponse = await callLLM(llmConfig, systemPrompt, userPrompt);
      console.log(`[PZ-BLOG] LLM response: ${rawResponse.length} chars`);
      ...
      console.log(`[PZ-BLOG] ✓ Blog created: slug="${blogSlug}" words=${totalWords}`);
      console.log(`[PZ-BLOG] ✓ Title: "${blog.title}"`);
      ...
      if (blogId) {
        ...
        console.log(`[PZ-BLOG] ✓ Blog promoted to published: id=${blogId}`);
      }
      ...
      for (const qRow of allRows) {
        await supabase(`puzzle?id=eq.${qRow.id}`, {
          method: 'PATCH',
          headers: { 'Prefer': 'return=minimal' },
          body: JSON.stringify({
            blog_linked:   true,
        ...
        }).catch(e => console.warn(`[PZ-BLOG] Could not set blog_linked on quiz ${qRow.id}: ${e.message}`));
      }
      console.log(`[PZ-BLOG] ✓ blog_linked=true set on ${allRows.length} quiz row(s)`);
      ...
      console.error(`[PZ-BLOG] FAILED for "${primaryQuiz.topic}": ${err.message}`);
      ...
      try {
        ...
        console.warn(`[PZ-BLOG] Could not insert error row: ${e2.message}`);
      }
    }
  }

  console.log(`\n[PZ-BLOG] Done. Generated ${generated} blog post(s).`);
}

run()
  .then(() => process.exit(0))
  .catch(err => { console.error('[PZ-BLOG] Fatal:', err); process.exit(1); });
