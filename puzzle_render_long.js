'use strict';
// ═══════════════════════════════════════════════════════════════
// PUZZLE RENDER LONG — Full-length Puzzle Long Format
//
// Clone of worker10.js (long trending renderer) with:
//   1. Polls `puzzle` table (long_status=pending_long)
//   2. Reads puzzle_template.html
//   3. Injects SVG via puzzleRenderers.js into {{PUZZLE_VISUAL}}
//   4. No Tavily photo overlay (always hidden)
//   5. Artifacts named *_puzzle_video.mp4
// ═══════════════════════════════════════════════════════════════
...
async function processJobs() {
  console.log('[PZ-LONG] Checking puzzle table...');
  ...
  try {
    ...
    const artifactPath = `/tmp/${quiz.id}_puzzle_video.mp4`;
    ...
    if (R2_CONFIGURED) {
      try {
        ...
        const videoKey = `puzzles/${quiz.id}.mp4`;
        await withTimeout(
          s3Client.send(new PutObjectCommand({
            Bucket: R2_BUCKET,
            Key: videoKey,
  ...
  if (!nicheNo) {
    try {
      const nicheCount = await fetchSupabase(
        `puzzle?niche=eq.${encodeURIComponent(niche)}&video_status=eq.rendered&id=neq.${quiz.id}&select=id`
      );
...
:root { --topic-photo-url: url("${overlayFileUrl}"); }
  ...
  // ── PUZZLE VISUAL INJECTION ─────────────────────────────────────────────
  let _puzzleSvgHtml = '';
  try {
    const { renderPuzzle } = require('./puzzleRenderers');
    const _spec  = typeof quiz.puzzle_spec === 'string'
      ? JSON.parse(quiz.puzzle_spec)
      : (quiz.puzzle_spec || {});
    const _type  = quiz.puzzle_type || 'number_sequence';
    const _popts = {
    ...
    const _pres = renderPuzzle(_type, _spec, _popts);
    _puzzleSvgHtml = _pres.svg || '';
    console.log(`[PZ-LONG] SVG rendered: type=${_type} bytes=${_puzzleSvgHtml.length} ok=${_pres.ok}`);
    if (!_pres.ok) console.warn('[PZ-LONG] SVG warnings:', _pres.warnings);
    if (_puzzleSvgHtml) {
      fetchSupabase(`puzzle?id=eq.${quiz.id}`, {
        method: 'PATCH',
        body: JSON.stringify({ puzzle_svg: _puzzleSvgHtml, updated_at: new Date().toISOString() })
      }).catch(e => console.warn('[PZ-LONG] SVG back-write failed:', e.message));
    }
  } catch (_e) {
    console.warn('[PZ-LONG] puzzleRenderers render failed:', _e.message);
  }

  R['{{PUZZLE_VISUAL}}'] = _puzzleSvgHtml;
