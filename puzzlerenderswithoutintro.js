'use strict';
// ═══════════════════════════════════════════════════════════════════════
// PUZZLE RENDER SHORT — 20-Second Puzzle Short Format with Avatar Strip
//
// IDENTICAL to worker10_short.js EXCEPT:
//   1. Polls the `puzzle` table (via puzzleAssigner.js)
//   2. Reads puzzle_template.html instead of quiz_template.html
//   3. Resolves puzzleRenderers.js to inject SVG into {{PUZZLE_VISUAL}}
//   4. No topic_image_url → background overlay is always hidden
//   5. Artifacts named *_puzzle_short_video.mp4
//
...
async function buildShortVideo(quiz, workDir) {
  ...
  if (quiz.puzzle_background_image_url && String(quiz.puzzle_background_image_url).startsWith('http')) {
    const dl = await download(quiz.puzzle_background_image_url, `sn_bgphoto_${quiz.id}`).catch(e => {
      console.warn(`[PZ-SHORT-BG] download() error: ${e.message.slice(0, 60)}`);
      return null;
    });
...
:root { --topic-photo-url: url("file://${dl}"); }
      ...
      videoPhotoClass = '';
      console.log(`[PZ-SHORT-BG] Photo overlay active: ${quiz.puzzle_background_image_url.slice(0, 70)}`);
    } else {
      console.log('[PZ-SHORT-BG] Download failed — overlay hidden (.no-photo).');
    }
  } else {
    console.log('[PZ-SHORT-BG] No puzzle_background_image_url on this row — overlay hidden.');
  }
  // ── Theme ──────────────────────────────────────────────────────────
  ...
  const R = {
    ...
    ...ctaVariantVars(quiz.id, { forceFollow: true }),
    '{{niche}}':           niche,
    '{{thumb_icon}}':      '❓',
...
async function processJobs() {
  ...
  const workDir = `/tmp/pz_short_${uuidv4()}`;
  ...
  try {
    ...
    const artifactPath = `/tmp/${quiz.id}_puzzle_short_nointro_video.mp4`;
    ...
    if (R2_CONFIGURED) {
      try {
        ...
        const key = `puzzles/${quiz.id}_short_nointro.mp4`;
        await withTimeout(
    ...
    await markPuzzleDone(patchForAssigner, quiz.id, cfg, shortVideoUrl);

    await fs.rm(workDir, { recursive: true, force: true }).catch(() => {});
    console.log(`[PZ-SHORT] Artifact: ${artifactPath}`);

  } catch (err) {
    console.error('[PZ-SHORT] FAILED:', err.message);
...
processJobs()
  .then(() => { console.log('[PZ-SHORT] Done.'); process.exit(0); })
  .catch(err => { console.error('[PZ-SHORT] Fatal:', err.message); process.exit(1); });
