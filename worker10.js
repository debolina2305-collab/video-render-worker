...
// TIGHTENED gaps to hit <50s target
const GAP_DEFAULT     = 0.25;   // was 0.35
const GAP_AFTER_STEP2 = 0.20;   // was 0.30
const GAP_OPTIONS     = 0.45;   // was 0.70
const GAP_ANSWER      = 0.35;   // was 0.50
// GAP_EXPL removed (explanation step removed)

// DEFAULT countdown — 10s keeps video under 50s
// DB value (quiz.thinking_time_sec) overrides this per row
...
// KEY FIX: download + convert every file to mp3/44100/stereo immediately
async function downloadAudio(url, cacheKey, preferKey) {
  ...
  try {
    ...
    await withTimeout(
      execPromise(`ffmpeg -y -i "${rawFile}" -ar 44100 -ac 2 -acodec libmp3lame -q:a 4 "${local}"`),
...
async function applyBgMusic(concatMp4, totalDur, voiceRanges, bgFile, workDir) {
  ...
  await ffmpeg(
    ...
    `-map "[a]" -ar 44100 -acodec libmp3lame "${mixedAudio}"`, 'mixAudio'
  );
  await ffmpeg(
    `-y -i "${concatMp4}" -i "${mixedAudio}" -c:v copy -map 0:v:0 -map 1:a:0 ` +
    `-c:a aac -b:a 192k -t ${totalDur} -movflags +faststart "${finalMp4}"`, 'remux'
  );
  return finalMp4;
}

// ──────────────────────────────────────────────
// THEME RESOLUTION + quiz_background_css
// ──────────────────────────────────────────────
...
async function processJobs() {
  ...
  if (stuckRows?.length) {
    ...
    for (const r of stuckRows)
      await fetchSupabase(`quiz?id=eq.${r.id}`,{method:'PATCH',body:JSON.stringify({video_status:'pending',updated_at:new Date().toISOString()})}).catch(()=>{});
  }
...
async function buildVideo(quiz, workDir) {
  ...
  // Use DB value if set, otherwise default to 10s (keeps video <50s)
  const QTIME    = (quiz.thinking_time_sec && quiz.thinking_time_sec > 0)
                   ? Math.min(quiz.thinking_time_sec, 12)  // hard cap at 12s for shorts
  ...
  const hasCta1 = !!(quiz.affiliate_url?.trim());
  ...
  const shot = async name => {
    ...
    await page.screenshot({path:p}); return p;
  };
  ...
  // ══ STEP 1: HOOK (min 2.5s) ══
  await showOnly('.hook-slide');
  await new Promise(r=>setTimeout(r,1100));
  const hookImg=await shot('hook');
  const hookAudio=await buildAudio({
    prerecorded:hookFile, fallbackText:quiz.hook_phrase||'Stop scrolling! Can you beat this?',
  ...
  // ══ STEP 2: Intro speech — text flash only, 1.5s, NO TTS ══
  await showOnly('.waiting-slide');
  await new Promise(r=>setTimeout(r,300));
  const waitImg=await shot('waiting');
  // Silence only (1.5s fixed) — no TTS, no prerecorded audio
  const waitSilPath=path.join(workDir,'wait_sil.mp3');
  await silence(1.5, waitSilPath);
  pushClip(await imgClip(waitImg, waitSilPath, 1.5, workDir, 'clip_wait'), false);

  // ══ STEP 3: Question appears (SFX + TTS) ══
  await showOnly('.question-appear-slide');
  await new Promise(r=>setTimeout(r,700));
  const qAppearImg=await shot('question_appear');
  // question_intro_audio_url plays first (audio only), then question TTS
  const step3Parts=[];
  if(questionIntroFile){
    step3Parts.push(questionIntroFile);
    const qiGap=path.join(workDir,'qi_gap.mp3'); await silence(0.2,qiGap);
    step3Parts.push(qiGap);
  }
  if(sfxFile){
    const sfxGap=path.join(workDir,'sfx_gap.mp3'); await silence(0.1,sfxGap);
    step3Parts.push(sfxFile,sfxGap);
  }
  const qTts=path.join(workDir,'q_tts.mp3');
  await tts(question,voice,qTts,3);
  step3Parts.push(qTts);
  ...
  // ══ STEP 4-5: Options (intro + TTS each + "time starts now") ══
  await showOnly('.question-static');
  await new Promise(r=>setTimeout(r,900));
  const optionsImg=await shot('options_static');
  const step45Parts=[];
  const gap4=path.join(workDir,'gap4.mp3'); await silence(GAP_OPTIONS,gap4); step45Parts.push(gap4);
  if(optionsIntroFile){ step45Parts.push(optionsIntroFile); }
  else { const oi=path.join(workDir,'ointro_tts.mp3'); await tts('And your options are',voice,oi,1.5); step45Parts.push(oi); }
  const gap5=path.join(workDir,'gap5.mp3'); await silence(0.2,gap5); step45Parts.push(gap5);
  if(sfxFile){ const sg2=path.join(workDir,'sfxgap2.mp3'); await silence(0.1,sg2); step45Parts.push(sfxFile,sg2); }
  for(let i=0;i<options.length;i++){
    if(!options[i]) continue;
    const oSil=path.join(workDir,`o_sil_${i}.mp3`); await silence(0.2,oSil);
  ...
  // "time starts now" TTS — kept per user request
  const startNowGap=path.join(workDir,'start_now_gap.mp3'); await silence(0.3,startNowGap);
  const startNowTts=path.join(workDir,'start_now.mp3');
  await tts(`You have only ${QTIME} seconds to crack the challenge — time starts now!`,voice,startNowTts,2);
  ...
  if(sfxFile){
    ...
    await ffmpeg(
      ...
      `-map "[a]" -t ${QTIME} -ar 44100 -acodec libmp3lame "${stingMixed}"`, 'cdStings'
    );
    cdFinal=stingMixed;
  }
  ...
  // ══ STEP 10: Answer reveal (no explanation — REMOVED step 11) ══
  await showOnly('.answer-slide');
  await new Promise(r=>setTimeout(r,300));
  const answerImg=await shot('answer');
  const step10Parts=[];
  const silReveal=path.join(workDir,'sil_reveal.mp3'); await silence(GAP_ANSWER,silReveal); step10Parts.push(silReveal);
  if(correctSfxFile){ step10Parts.push(correctSfxFile); const sg3=path.join(workDir,'sfxgap3.mp3'); await silence(0.15,sg3); step10Parts.push(sg3); }
  const correctTts=path.join(workDir,'correct_tts.mp3'); await tts(correct,voice,correctTts,1.5); step10Parts.push(correctTts);
  ...
  // ══ STEP 11: EXPLANATION REMOVED ══
  // Saves ~5s. No clip generated here.
  ...
  const ctaAudio=await buildAudio({
    ...
    fallbackText:hasCta1?(quiz.affiliate_text||'Check the link in description!'):(quiz.cta2_text||'Play real quiz and earn ONS tokens!'),
  ...
  if(quiz.mission_impossible_enabled!==false && quiz.mission_impossible_question){
    ...
    // STEP 14: MI intro (SFX + prerecorded/TTS)
    await showOnly('.mission-intro-slide');
    await new Promise(r=>setTimeout(r,400));
    const miIntroImg=await shot('mi_intro');
    const miParts=[];
    if(sfxFile){ const sg=path.join(workDir,'sfx_gap_mi.mp3'); await silence(0.15,sg); miParts.push(sg,sfxFile); const sg2=path.join(workDir,'sfx_gap_mi2.mp3'); await silence(0.2,sg2); miParts.push(sg2); }
    if(missionIntroFile){ miParts.push(missionIntroFile); }
    else { const mit=path.join(workDir,'mi_intro_tts.mp3'); await tts(quiz.mission_intro_text||'MISSION IMPOSSIBLE!',voice,mit,2); miParts.push(mit); }
    const miIntroAudio=path.join(workDir,'mi_intro_audio.mp3');
    await concatAudio(miParts,miIntroAudio,workDir);
    pushClip(await imgClip(miIntroImg,miIntroAudio,Math.max(await audioDur(miIntroAudio),2),workDir,'clip_mi_intro'));

    // STEP 15: MI question static (no TTS, 0.8s)
    await showOnly('.mission-final-slide');
    await page.evaluate(()=>{
      const h=document.getElementById('mi-hint'), c=document.getElementById('mi-cta3');
      if(h){h.classList.remove('shown');h.style.opacity='0';h.style.transform='scale(0.9)';}
    ...
    const miQSil=path.join(workDir,'mi_q_sil.mp3'); await silence(0.8,miQSil);
    pushClip(await imgClip(miQImg,miQSil,0.8,workDir,'clip_mi_q'),false);

    // STEP 16: Hint appears (SFX + 1.2s — tightened from 2.5s)
    await page.evaluate(()=>{
      const h=document.getElementById('mi-hint');
      if(h){h.classList.add('shown');h.style.opacity='';h.style.transform='';}
    });
    await new Promise(r=>setTimeout(r,400));
    const miHintImg=await shot('mi_hint');
    const mhParts=[]; if(sfxFile) mhParts.push(sfxFile);
    const mhSil=path.join(workDir,'mi_hint_sil.mp3'); await silence(1.2,mhSil); mhParts.push(mhSil);
    const miHintAudio=path.join(workDir,'mi_hint_audio.mp3');
    await concatAudio(mhParts,miHintAudio,workDir);
